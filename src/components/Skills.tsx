        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category, i) => (
            <motion.div
              ...
              className={`p-6 rounded-2xl border ${category.borderColor} bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-300 group`}
            >
              <h3
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill) => (
                  <span
                    key={skill}
                    className="px-3 py-1.5 text-sm rounded-lg bg-white/5 text-neutral-300 border border-white/5 hover:border-cyan-500/30 hover:text-cyan-400 transition-all duration-200"
                  >
                    {skill}
                  </span>
                ))}
